...
export default class extends Component {
    ...
    getData = (el) => {
        ...
        if (window.filters['id']) {
            const city = window.filters[id].city;

            const data = window.data[id].filter(item => item.city === city);
    ...
    rerender = (el) => {
        ...
        window.maps[id].destroy();

        ymaps.ready(async () => {
            window.maps[id] = new ymaps.Map(scene, options, {
                zoomControlPosition: { right: "10px", top: "105px" },
            })

            addMarkers(el,  window.maps[id]);
            centerize(el,  window.maps[id]);

            if (data.length > 10) {
                setOffset(el,  window.maps[id], offset);
            }
            setEvents(el,  window.maps[id]);

            await waitForTilesLoad( window.maps[id]);

            show(el, window.maps[id]);
